...
int WriteFile(int fd, char* arr, int isize)
{
	...
	if (((UFDTArr[fd].ptrfiletable->ptrinode->FileSize) - (UFDTArr[fd].ptrfiletable->ptrinode->FileActualSize)) < isize) return -2;
	...
	while (1)
	{
		...
		else if (count == 2)
		{
			...
			else if (strcmp(command[0], "write") == 0)
			{
				...
				if (ret > 0)
					printf("Success : %d bytes Succesfully written", ret);
			}
